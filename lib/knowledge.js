...
export function retrieveKnowledge(question, options = {}) {
  ...
  const isPortfolioDomainQuestion = /\b(omar|salama|portfolio|cv|github|project|experience|skill|education|certification|career|work)\b/.test(normalizedQuestion);
  const ranked = chunks
    .map(chunk => {
  ...
  const profileOrder = ['about', 'cv', 'professional-context', 'contact', 'experience', 'project', 'skill'];
  return profileOrder
