import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const rootDir = path.resolve(__dirname, '..');
const knowledgePath = path.join(rootDir, 'data', 'knowledge.json');

const STOP_WORDS = new Set([
  'a', 'about', 'after', 'all', 'also', 'am', 'an', 'and', 'any', 'are', 'as', 'at', 'be', 'built', 'by', 'can', 'did',
  'do', 'does', 'for', 'from', 'had', 'has', 'have', 'he', 'his', 'how', 'i', 'in', 'is', 'it', 'me', 'of', 'omar',
  'on', 'or', 's', 'salama', 'show', 'tell', 'that', 'the', 'this', 'to', 'use', 'used', 'uses', 'was', 'were', 'what',
  'when', 'where', 'which', 'who', 'why', 'with', 'would', 'ai', 'api', 'current', 'portfolio', 'test', 'today'
]);

export function loadKnowledge() {
  const raw = fs.readFileSync(knowledgePath, 'utf8');
  const parsed = JSON.parse(raw);
  return Array.isArray(parsed.chunks) ? parsed.chunks : [];
}

export function tokenize(text = '') {
  const tokens = String(text)
    .toLowerCase()
    .replace(/[^a-z0-9+#.]+/g, ' ')
    .split(/\s+/)
    .filter(token => token.length > 1 && !STOP_WORDS.has(token) && !/^\d+$/.test(token));
  if (tokens.includes('agentic')) tokens.push('agent', 'multi', 'multi-agent');
  if (tokens.includes('study') || tokens.includes('studied')) tokens.push('education', 'degree', 'university');
  if (tokens.includes('technologies') || tokens.includes('technology')) tokens.push('skill', 'skills');
  if (tokens.includes('agents')) tokens.push('agent');
  return tokens;
}

function chunkText(chunk) {
  return [
    chunk.title,
    chunk.type,
    chunk.source,
    chunk.content,
    chunk.metadata?.technologies?.join(' '),
    chunk.metadata?.topics?.join(' ')
  ].filter(Boolean).join(' ');
}

export function retrieveKnowledge(question, options = {}) {
  const chunks = options.chunks || loadKnowledge();
  const limit = options.limit || 7;
  const queryTokens = tokenize(question);
  const querySet = new Set(queryTokens);
  const normalizedQuestion = String(question).toLowerCase();
  const asksLocation = /\b(where|location|live|based)\b/.test(normalizedQuestion) &&
    !/\b(study|studied|education|university|degree)\b/.test(normalizedQuestion);
  const asksProjectTimeline = /\b(last|latest|newest|recent)\b.*\bprojects?\b|\bprojects?\b.*\b(last|latest|newest|recent)\b/.test(normalizedQuestion);
  const asksPortfolioOverview = /\b(?:check|review|overview|portfolio)\b/.test(normalizedQuestion);
  const mentionsOmar = /\bomar\b/.test(normalizedQuestion);

  if (asksLocation && mentionsOmar) {
    return chunks
      .filter(chunk => ['contact', 'about', 'resume'].includes(chunk.type))
      .sort((left, right) => (left.type === 'contact' ? -1 : 0) - (right.type === 'contact' ? -1 : 0))
      .slice(0, limit);
  }

  if (asksProjectTimeline) {
    return chunks
      .filter(chunk => (chunk.type === 'project' && chunk.title !== 'Featured Projects') || chunk.type === 'resume')
      .slice(0, limit);
  }

  if (asksPortfolioOverview || (mentionsOmar && queryTokens.length === 0)) {
    return chunks
      .filter(chunk => ['about', 'experience', 'project', 'skill', 'resume'].includes(chunk.type))
      .slice(0, limit);
  }

  if (queryTokens.length === 0 && !normalizedQuestion.includes('tell me about omar')) {
    return [];
  }

  if (normalizedQuestion.includes('tell me about omar')) {
    return chunks
      .filter(chunk => ['about', 'resume', 'experience', 'skill'].includes(chunk.type))
      .slice(0, limit);
  }

  return chunks
    .map(chunk => {
      const text = chunkText(chunk);
      const normalizedText = text.toLowerCase();
      const tokens = tokenize(text);
      const tokenSet = new Set(tokens);
      const overlap = queryTokens.filter(token => tokenSet.has(token)).length;
      const phraseBoost = queryTokens.reduce((score, token) => {
        return score + (normalizedText.includes(token) ? 0.35 : 0);
      }, 0);
      const titleBoost = tokenize(chunk.title).filter(token => querySet.has(token)).length * 2;
      const typeBoost = queryTokens.includes(chunk.type) ? 1.5 : 0;
      const sourceBoost = chunk.source === 'portfolio' ? 0.25 : 0;
      let intentBoost = 0;

      if (normalizedQuestion.includes('shifaa') && normalizedText.includes('shifaa')) intentBoost += 6;
      if (normalizedQuestion.includes('computer vision') && normalizedText.includes('computer vision')) intentBoost += 4;
      if (normalizedQuestion.includes('certification') && chunk.type === 'certification') intentBoost += 5;
      if ((normalizedQuestion.includes('study') || normalizedQuestion.includes('education')) && normalizedText.includes('university')) intentBoost += 4;
      if ((normalizedQuestion.includes('technolog') || normalizedQuestion.includes('skills')) && chunk.type === 'skill') intentBoost += 5;
      if (normalizedQuestion.includes('project') && chunk.type === 'project') intentBoost += 4;
      if (/(hire|suitable|position|role|recruit)/.test(normalizedQuestion) && ['about', 'experience', 'project', 'skill'].includes(chunk.type)) {
        intentBoost += 3;
      }
      if (normalizedQuestion.includes('github') && chunk.source === 'github') intentBoost += 6;
      if (chunk.type === 'contact' && !normalizedQuestion.includes('contact') && !normalizedQuestion.includes('email') && !normalizedQuestion.includes('phone')) {
        intentBoost -= 4;
      }

      return { chunk, score: overlap + phraseBoost + titleBoost + typeBoost + sourceBoost + intentBoost, overlap, titleBoost, intentBoost };
    })
    // Do not retrieve a portfolio page solely because it is from the portfolio.
    // A source must share a meaningful query token or receive an intent boost.
    .filter(item => item.overlap >= 2 || item.titleBoost > 0 || item.intentBoost > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, limit)
    .map(item => item.chunk);
}

function asksForResources(question) {
  return /\b(cv|resume|curriculum vitae|github|repo|repository|code|link|links|resource|resources|demo|paper)\b/i.test(question);
}

function asksForProjects(question) {
  return /\b(project|projects|computer vision|machine learning|ai engineer|hire|suitable)\b/i.test(question);
}

export function selectRelevantResources(question, chunks) {
  const normalized = String(question || '').toLowerCase();
  const resources = [];

  if (/\b(cv|resume|curriculum vitae)\b/i.test(normalized)) {
    resources.push({ type: 'cv', title: 'Omar Salama CV', url: '/Omar_Salama_CV.pdf' });
  }

  const explicit = asksForResources(normalized);
  const projects = asksForProjects(normalized);
  if (explicit || projects) {
    chunks
      .filter(chunk => chunk.url && (
        (normalized.includes('github') && chunk.source === 'github') ||
        (normalized.includes('repo') && (chunk.type === 'project' || chunk.source === 'github')) ||
        (normalized.includes('demo') && chunk.type === 'project') ||
        (normalized.includes('paper') && chunk.type === 'project') ||
        (projects && (chunk.source === 'github' || (chunk.type === 'project' && chunk.title !== 'Featured Projects')))
      ))
      .forEach(chunk => resources.push({
        type: chunk.source,
        title: chunk.title,
        url: chunk.url
      }));
  }

  const seen = new Set();
  return resources.filter(resource => {
    if (seen.has(resource.url)) return false;
    seen.add(resource.url);
    return true;
  }).slice(0, 4);
}

export function formatContext(chunks, options = {}) {
  const maxChars = options.maxChars || Infinity;
  const maxChunkChars = options.maxChunkChars || Infinity;
  const includeUrls = options.includeUrls !== false;
  let remaining = maxChars;

  return chunks.reduce((context, chunk, index) => {
    if (remaining <= 0) return context;

    const url = includeUrls && chunk.url ? `\nURL: ${chunk.url}` : '';
    const content = String(chunk.content || '').slice(0, maxChunkChars);
    const entry = `[${index + 1}] Source: ${chunk.source}; Type: ${chunk.type}; Title: ${chunk.title}${url}\n${content}`;
    const trimmed = entry.slice(0, remaining);
    remaining -= trimmed.length + 2;
    return context ? `${context}\n\n${trimmed}` : trimmed;
  }, '');
}

export function uniqueSources(chunks) {
  const seen = new Set();
  return chunks
    .map(chunk => ({
      type: chunk.source,
      title: chunk.title,
      url: chunk.url || undefined
    }))
    .filter(source => {
      const key = `${source.type}:${source.title}:${source.url || ''}`;
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    });
}
